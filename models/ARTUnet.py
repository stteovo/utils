...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W, mask=None, mask_ratio=0.):
        """
        Args:
...
class Downsample(nn.Module):
    def __init__(self, n_feat):
        ...
        self.body = nn.Sequential(nn.Conv2d(n_feat, n_feat // 2, kernel_size=3, stride=1, padding=1, bias=False),
                                  nn.PixelUnshuffle(2))
        # self.body = nn.Sequential(nn.Conv2d(n_feat, n_feat * 2, kernel_size=4, stride=2, padding=1, bias=False),)
...
class ARTUNet(nn.Module):
    def __init__(self,
        ...
        self.down0_1 = Downsample(dim // 2)  ## From Level 0 to Level 1
        ...
        self.decoder_level1 = nn.ModuleList([
            TransformerBlock(dim=int(dim), num_heads=heads[0], window_size=window_size[0],
                             interval=interval[0],
                             ds_flag=0 if i % 2 == 0 else 1,
                             mlp_ratio=mlp_ratio,
                             qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        self.up1_0 = Upsample(int(dim))  ## From Level 2 to Level 1  (NO 1x1 conv to reduce channels)
        ...
        self.refinement = nn.Sequential(
            nn.Conv2d(int(dim), int(dim // 2), 3, 1, 1, bias=bias),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
        )

        self.output_rgb = nn.Conv2d(int(dim // 2), 3, kernel_size=3, stride=1, padding=1, bias=bias)
        self.output_alpha = nn.Conv2d(int(dim // 2), 1, kernel_size=3, stride=1, padding=1, bias=bias)

        # self.apply(init_weights_kaiming)

    def forward(self, x_in, mask):
        inp_img = torch.cat([x_in, mask], dim=1)

        _, _, H, W = inp_img.shape
        H, W = H // 2, W // 2
        inp_enc_level1 = self.patch_embed(inp_img)  # b,hw,c
        out_enc_level1 = self.down0_1(inp_enc_level1, H * 2, W * 2)
        ...
        inp_dec_level1 = rearrange(inp_dec_level1, "b c h w -> b (h w) c").contiguous()  # b, hw//4, 2c
        out_dec_level1 = inp_dec_level1
        for layer in self.decoder_level1:
            out_dec_level1 = layer(out_dec_level1, [H, W])

        inp_dec_level0 = self.up1_0(out_dec_level1, H, W)
        # out_dec_level0 = torch.cat([inp_dec_level0, inp_dec_level0], 2)
        out_dec_level0 = torch.cat([inp_dec_level0, inp_enc_level1], 2)
        out_dec_level0 = rearrange(out_dec_level0, "b (h w) c -> b c h w", h=H * 2, w=W * 2).contiguous()
        out_dec_level0 = self.refinement(out_dec_level0)
        # for layer in self.refinement:
        #     out_dec_level0 = layer(out_dec_level0, [H * 2, W * 2])

        rgb = self.output_rgb(out_dec_level0)
        mask_pre = self.output_alpha(out_dec_level0)
        mask_out = torch.sigmoid(mask_pre) if self.b_sigmoid else (mask_pre + 1) / 2

        if self.b_testing:
            return rgb, mask_out

        res = rgb * mask_out + x_in * (1 - mask_out)
        return rgb, mask_out, res, mask_pre

class ARTUNet_inc5(nn.Module):
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
                 dim=48,
        ...
        super(ARTUNet_inc5, self).__init__()
        self.b_testing = b_testing
        self.b_sigmoid = b_sigmoid
        self.patch_embed = OverlapPatchEmbed(inp_channels, dim // 2)
    ...
    def forward(self, x_in, mask, mask1):
        inp_img = torch.cat([x_in, mask, mask1], dim=1)
...
class ARTUNet_S(nn.Module):
    ...
    def forward(self, x_in, mask, b_sigmoid=True):
        inp_img = torch.cat([x_in, mask], dim=1)
        ...
        inp_dec_level2 = self.up3_2(out_enc_level3, H // 4, W // 4)  # b, hw//4, 2c
        inp_dec_level2 = torch.cat([inp_dec_level2, out_enc_level2], 2)
        inp_dec_level2 = rearrange(inp_dec_level2, "b (h w) c -> b c h w", h=H // 2, w=W // 2).contiguous()
        inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2)
        inp_dec_level2 = rearrange(inp_dec_level2, "b c h w -> b (h w) c").contiguous()  # b, hw//4, 2c
        ...
        if b_sigmoid:
            mask_out = torch.sigmoid(mask_pre)
        else:
            mask_out = (mask_pre + 1) / 2

        res = rgb * mask_out + x_in * (1 - mask_out)
        return rgb, mask_out, res, mask_pre


class ARTUNet_input(ARTUNet):
    def forward(self, inp_img):
        _, _, H, W = inp_img.shape
        H, W = H // 2, W // 2
        inp_enc_level1 = self.patch_embed(inp_img)  # b,hw,c
        ...
        rgb = self.output_rgb(out_dec_level0) + inp_img
        alpha = self.output_alpha(out_dec_level0)
        bias = self.output_bias(out_dec_level0)

        # rgb = torch.tanh(rgb)
        # alpha = torch.tanh(alpha)
        alpha = torch.clip(alpha, -1, 1)
        # bias = torch.tanh(bias)

        # alpha = torch.clip(alpha, -1, 1)

        return rgb, alpha, bias


class ARTUNet_Seg(nn.Module):
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
                 dim=48,
        ...
        super(ARTUNet_Seg, self).__init__()
        ...
        self.output_mask = nn.Conv2d(int(dim // 2), 3, kernel_size=3, stride=1, padding=1, bias=bias)
    ...
    def forward(self, inp_img):
        ...
        mask = self.output_mask(out_dec_level0)
...
if __name__ == '__main__':
    ...
    input = torch.rand((1, 3, 128, 512))  # + 192 * 4
    mask = torch.rand((1, 1, 128, 512))
    mask2 = torch.rand((1, 1, 128, 512))

    # net = ARTUNet_inc5(inp_channels=5,
    #                 out_channels=4,
    #                 dim=32,
    #                 num_blocks=[1, 1, 1, 1],
    #                 num_refinement_blocks=4,
    #                 heads=[1, 2, 4, 8],
    #                 window_size=[8, 8, 8, 8], mlp_ratio=2.,
    #                 qkv_bias=True, qk_scale=None,
    #                 interval=[32, 16, 8, 4],
    #                 # interval=[8, 8, 4, 4],
    #                 bias=False, dual_pixel_task=False,
    #                 b_sigmoid=True, b_testing=True,
    #                )
    net = ARTUNet(inp_channels=4,
                    out_channels=4,
                    dim=32,
                    num_blocks=[1, 1, 1, 1],
                    num_refinement_blocks=4,
    ...
    #               b_testing=True,
    #               )
    # net = ARTUNet_S(inp_channels=4,
    #                  out_channels=4,
    #                  dim=48,
    #                  num_blocks=[1, 1, 2],
    #                  num_refinement_blocks=4,
    #                  heads=[1, 4, 8],
    #                  window_size=[8, 8, 8], mlp_ratio=2,
    #                  qkv_bias=True, qk_scale=None,
    #                  interval=[32, 16, 8],
    #                  )
    ...
    # net(input, mask, mask2)
    # inputs = torch.rand(2, 5, 144, 144)
    # net(inputs)
    ...
    if True:
        ...
        # do_export('/root/group-trainee/ay/face_cut_v1_1120.onnx', net, in_c=5, shape=[128, 512], cuda=False
        #           , checkpoint='/data_ssd/ay/checkpoint/neck_color/Artnet_v4/latest_net_0.pth')

        # inputs = torch.rand(2, 5, 144, 144)
        # do_export_batch('/root/group-trainee/ay/face_cut_v2.onnx', net, shape=[144, 144], cuda=False
        #           , checkpoint='/data_ssd/ay/checkpoint/neck_color/Artnet_v2/latest_net_0.pth')