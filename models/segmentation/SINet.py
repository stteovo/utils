...
class SINet_Encoder(nn.Module):
    ...
    def __init__(self, config, in_c=3,classes=20, p=5, q=3,  chnn=1.0):
        '''
...
class SINet(nn.Module):
    ...
    def __init__(self, config, in_c=3, classes=20, p=2, q=3, chnn=1.0, encoderFile=None, sigmoid=False):
        '''
        :param classes: number of classes in the dataset. Default is 20 for the cityscapes
        :param p: depth multiplier
        :param q: depth multiplier
        ...
        self.encoder = SINet_Encoder(config, in_c, classes, p, q, chnn)
    ...
    # def forward(self, input):
    def forward(self, img, mask, mask2):
        input = torch.cat([img, mask, mask2], dim=1)
        ...
        seg_map = self.classifier(Dnc_stage2)

        return seg_map

class SINet_for_handSeg(nn.Module):
...
def export():
    ...
    input = torch.rand((1, 3, 768, 768))  # + 192 * 4
    mask = torch.rand((1, 1, 768, 768))
    mask2 = torch.rand((1, 1, 768, 768))
    config = [[[3, 1], [5, 1]], [[3, 1], [3, 1]],
              [[3, 1], [5, 1]], [[3, 1], [3, 1]], [[5, 1], [3, 2]], [[5, 2], [3, 4]],
              [[3, 1], [3, 1]], [[5, 1], [5, 1]], [[3, 2], [3, 4]], [[3, 1], [5, 2]]]
    model = SINet(in_c=5, classes=2, p=2, q=8, config=config,
                  chnn=1, sigmoid=True)
    model.eval()

    start_time = time.perf_counter()
    model(input, mask, mask2)
    ...
    if True:
        ...
        # do_export('/root/group-trainee/ay/arcnet_b1.onnx', net, in_c=4, shape=[512, 512], cuda=False
        #           , checkpoint='/data_ssd/ay/checkpoint/neck_color/artunet_v7_b1/latest_net_0.pth')
        do_export('/root/group-trainee/ay/face_cut_seg_1209_完全版.onnx', model, in_c=5, shape=[768, 768], cuda=False
                  , checkpoint='/data_ssd/ay/切脸数据/checkpoints/seg_v1_woLimitMask/keep/facenet_v3/epoch_135_net_0_完全版.pth')
...
#
#
#     model = SINet(in_c=5, classes=1, p=2, q=8, config=config,
#                   chnn=1)
#     model.eval()
#
#     batch = torch.randn(1, 5, 768, 768)
...
#         batch = batch.cuda()  # .half()	#HALF seems to be doing slower for some reason
#
#     time_train = []
#     i = 0
#
...
#             fwt = time.perf_counter() - start_time
