...
class Syatt(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer=nn.BatchNorm2d, auto_pad=False, dim=64, topK=1):
        ...
        self.partial_op = PartialConv2d(inter_channels, inter_channels, kernel_size=5, stride=1, padding=3, bias=False,
                                        multi_channel=True)
    ...
    def forward(self, x, y, mask):
        ...
        attn_weight = (
                                  q_pix * self.scale) @ k_pix_sel  # (n*p^2, w^2, c) @ (n*p^2, c, topk*h_kv*w_kv) -> (n*p^2, w^2, topk*w2)

        if mask_windows is not None:
            _, _, C = attn_weight.shape
            attn_mask_windows = mask_windows.expand(-1, -1, C)
            attn = attn_weight + attn_mask_windows.masked_fill(attn_mask_windows == 0, float(-100.0)).masked_fill(
        ...
        out = rearrange(out, '(n j i) (h w) c -> n (j h) (i w) c', j=self.n_win, i=self.n_win, h=H // self.n_win,
                        w=W // self.n_win)
